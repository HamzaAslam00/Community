...
<form action="{{ $url }}" class="gy-3 form-settings form-validate is-alter" data-form="ajax-form" method="post"
    data-modal="#ajax_model" data-datatable="#tickets_table">
    @csrf
    @if($isEdit)
        @method('put')
    @endif
            ...
            <div class="form-group">
                ...
                <div class="form-control-wrap">
                    <textarea type="text" class="form-control" id="description"
                        name="description">{{ $isEdit ? $ticket->description : '' }}</textarea>
                </div>
            </div>
        </div>
        <div class="col-lg-6">
            <div class="form-group">
                ...
                <select class="form-control form-select" id="status" name="status" required>
                    ...
                    <option value="inactive" @if($isEdit && $ticket->status == 'inactive') selected @endif>In-Active
                    </option>
                </select>
            </div>
        </div>
